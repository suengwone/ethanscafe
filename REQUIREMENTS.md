...
- ✅ **알림함** (`/notifications`) — 보낸 푸시를 앱 안에서 다시 본다
  ...
  - 항목 id는 `주문id:상태`라 트리거가 두 번 돌아도 겹치지 않고, 이미 있는 항목은 그대로 둬 사용자가 표시한 읽음을 되돌리지 않는다
...
- ✅ 설정: 알림함 (`/notifications`), 화면 테마 (`/profile/appearance`), 알림 설정, 결제 수단, 배송지 관리 (`/profile/addresses`)
