...
| ★★★ | 선불권 포인트 충전 | 정액 선불권 결제 시 포인트 충전 + 보너스 지급 (상세 기획: 6.3) | 포인트, 토스페이먼즈, Cloud Functions |
| ★★ | ~~재주문~~ ✔ 구현 완료 | 주문 내역에서 동일 구성으로 장바구니 담기 (원두·픽업 주문 모두, 판매 종료 상품 제외 안내) | `features/order` `reorder.dart` |
| ★★ | ~~멤버십 등급제~~ ✔ 구현 완료 | 누적 결제 기반 등급(그린 10% / 골드 30만원↬ 12% / 플래티넘 100만원↬ 15%) 적립률 차등 | `features/points` `membership_tier.dart` |
...
- 충전 결제 금액은 멤버십 등급 산정용 누적 결제 금액에 **포함하지 않는다** (충전한 포인트로 상품 결제 시 이중 집계 방지, 실제 상품 결제 시점에만 집계)
