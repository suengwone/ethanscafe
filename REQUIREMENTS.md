...
> 출시에 걸린 것들은 `RELEASE.md`에 무엇이 왜 막혀 있는지 함께 적어 두었다.

1. ⬜ 이용약관·개인정보처리방침의 영문본 — 법률 검토를 거친 번역문이 있어야 넣을 수 있다 (§4.16 참고)
2. ⬜ 토스페이먼츠 운영 키 발급 및 등록 (미설정 시 테스트 키·모의 결제로 동작)
3. ⬜ 매장 지도 SDK 도입 검토 (네이버/카카오 지도 — API 키 및 네이티브 설정 필요)
4. ⬜ `store_url` 채우기 — 스토어에 앱을 올린 뒤 android/ios 조건을 붙인다. 지금은 비어 있어 업데이트 안내의 이동 버튼이 숨는다
5. ⬜ 개인정보처리방침이 코드보다 좁다 — 위치·FCM 토큰·결제 식별자·카메라가 빠져 있다. 스토어 신고와 어긋나므로 법률 검토 후 넣어야 한다 (`RELEASE.md` §2)
6. ⬜ iOS 서명 설정 (`DEVELOPMENT_TEAM`이 비어 있어 아카이브가 되지 않는다)
...
| 2026-08-31 | 출시 준비 — `RELEASE.md` 신설. 스토어 심사가 요구하는 방침 주소를 위해 이용약관·개인정보처리방침을 로그인 없이 열리게 하고, iOS 홈 화면 이름이 `Cafe App`으로 뜨던 것을 `폭스트롯`으로 맞췄다. 데이터 보안 양식에 적을 값을 코드에서 정리했고, 방침이 코드보다 좁다는 것(위치·푸시 토큰·결제 식별자·카메라 누락)을 백로그로 올렸다 |
