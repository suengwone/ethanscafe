...
- ✅ 카탈로그 관리 (`/points/catalog`) — 탭 4종
  - 메뉴 · 원두: 등록·수정·내리기 + 목록에서 품절 토글
  - 배너: 홈 이벤트 배너 등록·수정·내리기 (제목·설명·아이콘·노출 순서)
  - 매장: 매장 등록·수정·내리기 (주소·전화·좌표·영업시간·편의시설·노출 순서)
  - 목록 조회가 `sortOrder`로 정렬하므로 저장 시 이 필드를 **항상 함께 쓴다**. 값이 없는 문서는
    쿼리에서 통째로 빠져 앱에 보이지 않는다
...
| `/points/catalog` | 관리자 카탈로그 관리 (메뉴·원두·배너·매장 등록·수정) | 페이 |
| `/profile` | 내 정보 | 마이 |
...
6. ⬜ 공지(`notices`) 등록/수정 도구 — 카탈로그 관리에 남은 마지막 마스터 데이터. 아직 콘솔에서 문서를 직접 만들어야 한다
