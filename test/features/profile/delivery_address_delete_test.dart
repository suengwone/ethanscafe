...
void main() {
  ...
  testWidgets('삭제를 고르면 확인 다이얼로그를 먼저 보여준다', (tester) async {
    ...
    expect(find.text('배송지 삭제'.keepWord), findsOneWidget);
    expect(find.byType(Card).evaluate().length, before);
  });

  testWidgets('확인 다이얼로그에서 취소하면 배송지가 남는다', (tester) async {
    await pumpScreen(tester);
    final before = find.byType(Card).evaluate().length;
    ...
    expect(find.text('배송지 삭제'.keepWord), findsNothing);
    expect(find.byType(Card).evaluate().length, before);
  });

  testWidgets('확인 다이얼로그에서 삭제하면 배송지가 지워지고 안내가 보인다', (tester) async {
    await pumpScreen(tester);
    final before = find.byType(Card).evaluate().length;
    ...
    expect(find.text('배송지를 삭제했어요.'), findsOneWidget);
  });
}
